...
These are behind feature flags and don't compile by default:

```sh
# Python (requires Python + maturin)
cd crates/gsem-py && maturin develop --release --features pyo3

# R (requires R + extendr toolchain)
cargo build --features gsem-r/extendr
```
